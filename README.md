...
- Git URL から追加すると、自動で `Assets/IGNORANZ-PROJECT/` に展開されます
...
5. `Add` を押して取り込みます。
6. 取り込み後、CollabSync は自動で `Assets/IGNORANZ-PROJECT/` に展開されます。
7. スクリプトの再コンパイルが終わったら、Unity メニューに `Tools > CollabSync` が出ていることを確認します。

Git URL 経由のインストールでも、最終的な配置先は `Assets/IGNORANZ-PROJECT/` です。
外部パッケージの追加、別サービスのアカウント作成は不要です。

#### 代替: 手動で配置する

1. このリポジトリ内の `IGNORANZ-PROJECT` フォルダを、Unity プロジェクトの `Assets/` 直下に配置します。
   `Assets/IGNORANZ-PROJECT/Editor` と `Assets/IGNORANZ-PROJECT/Runtime` になる形です。
...
2. ZIP 展開後などでフォルダ名が変わっている場合は、最終的に `IGNORANZ-PROJECT` になるように揃えてください。
...
正常に取り込めれば、CollabSync は自動で `Assets/IGNORANZ-PROJECT/` に展開されます。
