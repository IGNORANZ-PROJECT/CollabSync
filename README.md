...
チームで 1 つの JSON パスを共有するだけで、次の情報を同期できます。
...
- 共有方法は「同じ JSON パスを全員が指定する」だけです
- OneDrive などの同期ストレージでは、内部的に sidecar の `*.collabsync-store/` に小さい record 単位で保存し、1 ファイル全体の上書きを避けます
- 自動ロックと手動ロックの両方に対応しています
- 共有 JSON のバックアップを自動で作成し、古いものは自動整理します
- 共有 JSON 自体の書き込み競合を自動回避します
- 共有状態ファイルは保護形式で保存され、平文の直接編集や改ざんを検知しやすくしています
- `User ID` ベースでユーザーを識別するため、表示名を変えても別ユーザー扱いになりません

## 機能一覧

- オンラインユーザー表示
  だれが現在オンラインか、何を触っているかを確認できます。
- 自動ロック
  Scene / Prefab / Object / Asset / Script の編集を検知し、必要最小限の範囲で自動ロックします。
- 手動ロック
  Project や Hierarchy から明示的にロック / 解除できます。
- チームメモ
  Markdown 対応メモ、ピン留め、既読、選択対象への紐付けに対応しています。
- 作業履歴
  ユーザーごとの最近の編集履歴を確認できます。
- 管理者機能
  Admin 追加 / 削除、Root Admin によるユーザー削除と復活に対応しています。
- Doctor
  共有状態ファイルの作成確認、読み書き確認、異常検知を行えます。
- 共有状態ファイル保護
  shared state file は保護形式で保存され、平文の直接編集や改ざんを検知しやすくしています。
- Git-aware Locks
  必要に応じて retained lock を使い、保護ブランチ反映までロック保持を補助できます。
...
CollabSync を開く場所は Unity の上部メニューバーです。

- macOS: 画面最上部のアプリメニューバーにある `Tools > CollabSync`
- Windows: Unity ウィンドウ上部のメニューバーにある `Tools > CollabSync`

`Tools` が見つからない場合は、CollabSync の導入が完了していないか、コンパイルエラーでメニュー登録が止まっている可能性があります。

## 2. 共有ファイルの準備

CollabSync は、チーム全員が同じ JSON パスを使うことで同期します。
...
- チーム全員が **同じ内容に同期される同じ JSON パス / store** を指定する必要があります

補足:

- `JSON Path` で指定するのは引き続き 1 つの `.json` パスです
- `0.2.x` 系の新しい方式では、そのパス自体は entry point / manifest として使い、実際の共有更新は横に作られる `*.collabsync-store/` 配下の小さい record ファイル群へ分散して書き込みます
- そのため、OneDrive などでも毎回 state 全体を 1 ファイル上書きする方式より競合しにくくなっています
...
共有ファイルが 1 つでも、各メンバーは自分の PC から見えるローカルパスで指定します。

例:

1. Root Admin が OneDrive や NAS の共有フォルダに `state.json` を作成します
2. 他メンバーは、その共有フォルダを自分の PC に同期またはマウントします
3. 各メンバーが Unity で `Tools > CollabSync > Settings` を開きます
4. `JSON Path` の `Choose...` を押します
5. 自分の PC 上で同期されている `state.json` を選びます
6. `Resolved Local JSON` に、自分の環境での実際のローカルパスが表示されていることを確認します

重要なのは、全員が「同じ共有ストレージの実体」を指すことです。
パス文字列自体は OS やユーザー名の違いで一致していなくて構いません。

やってはいけない例:

- ブラウザの共有リンクをそのまま貼る
- `https://...` の URL を指定する
- OneDrive / Dropbox / Google Drive の Web 画面の URL を指定する
- 共有フォルダ内の別コピーを各自が選ぶ
...
OneDrive などを使う場合は、「共有リンク」ではなく「どの共有フォルダ内の `state.json` を選ぶか」を伝えてください。
...
ここで選ぶのは、自分の PC に同期されているローカルファイルです。
たとえば Windows と macOS で表示パスが違っても、同じ OneDrive / NAS 上の同じ `state.json` なら問題ありません。
...
- `User ID` はローカル保護ストレージに固定保存され、`Settings` から直接編集できません
...
また、通常一覧の下にある `Deleted Users` を開くと、削除済みユーザー一覧を確認し、必要なユーザーだけ復活できます。
...
`Your User ID` は参照専用です。CollabSync はこれをローカルの保護ファイルへ保存し、以前の ID を再利用して勝手に変わりにくくしています。
...
共有状態ファイルの実体は、保存時に保護形式へ変換されます。
そのため、中身をそのままテキスト編集しても正しく反映されず、改ざんが検出された場合は Doctor やバックエンド側で異常として扱います。
また、OneDrive などの同期ストレージでは、この `.json` パスの横にある `*.collabsync-store/` へ record 単位で分散保存し、1 ファイル丸ごとの上書きを避けます。
...
- Root Admin は `Deleted Users` 一覧からあとで復活できます
...
実際の共有状態ファイルは `0.2.6` 以降、保護形式で保存されます。
以下は on-disk の暗号化ラッパーを外したあとに扱う論理構造の例です。
...
- README に `Tools > CollabSync` の場所説明を追加
- README に共有ファイルを各 PC でローカルパスとして指定する手順を追加
- README に主要機能をまとめた `機能一覧` を追加

### ver.0.2.6

- Root Admin 専用の `Deleted Users` 一覧を追加し、削除済みユーザーを一覧から復活できるように改善
- `User ID` をローカル保護ストレージへ固定保存するように変更し、`Settings` から直接変えられないように調整
- 共有状態ファイルを保護形式で保存するように変更し、平文の直接編集や改ざんを検知しやすく改善
- `Doctor` も保護形式の shared state file を正しく検査・読み書きできるように更新
