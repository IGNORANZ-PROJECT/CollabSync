...
public static class EditingTracker
{
    ...
    static void CheckGitHeadChange()
    {
        ...
        _lastGitHeadSignature = currentSignature;
        _ = SafeReleaseAllAutoLocksOnCommit();
    }

    static async Task SafeReleaseAllAutoLocksOnCommit()
    {
        try { await ReleaseAllAutoLocksOnCommitAsync(); }
        catch (Exception e) { Debug.LogError($"[CollabSync] Auto lock release on commit error: {e}"); }
    }
