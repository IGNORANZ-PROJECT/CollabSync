...
{
    [MenuItem("Tools/CollabSync/Lock Current Selection", priority = 21)]
    public static void LockCurrentSelection()
    {
        if (!TryGetBackend(out var backend)) return;
        if (!CollabSyncEditorLockUtility.TryGetCurrentLockTarget(out var target)) return;

        var meId = CollabSyncUser.UserId;
        var meName = CollabSyncUser.UserName;
        if (string.IsNullOrEmpty(target.lockKey) || (string.IsNullOrEmpty(meId) && string.IsNullOrEmpty(meName))) return;
        _ = backend.TryAcquireLockAsync(target.lockKey, meId, meName, reason: "tools-menu", ttlMs: 0);
    }

    [MenuItem("Tools/CollabSync/Unlock Current Selection", priority = 22)]
    public static void UnlockCurrentSelection()
    {
        if (!TryGetBackend(out var backend)) return;
        if (!CollabSyncEditorLockUtility.TryGetCurrentLockTarget(out var target)) return;
        ...
        var meName = CollabSyncUser.UserName;
        if (string.IsNullOrEmpty(target.lockKey) || (string.IsNullOrEmpty(meId) && string.IsNullOrEmpty(meName))) return;
        _ = backend.ReleaseLockAsync(target.lockKey, meId, meName);
    }

    [MenuItem("Tools/CollabSync/Lock Current Selection", validate = true)]
    [MenuItem("Tools/CollabSync/Unlock Current Selection", validate = true)]
    private static bool ValidateCurrentSelectionLockCommand()
    {
        return CollabSyncEditorLockUtility.TryGetCurrentLockTarget(out var target)
            && target != null
            && !string.IsNullOrEmpty(target.lockKey);
    }
